  ...
  return (
    <div className="settings-section">
      ...
      <div className="settings-logo-upload" onClick={() => inputRef.current.click()}>
        <input ref={inputRef} type="file" accept="image/png,image/jpeg" style={{ display: "none" }} onChange={e => { const f = e.target.files[0]; if (f) setLogo(URL.createObjectURL(f)); }} />
        {logo ? <img src={logo} alt="Logo" className="settings-logo-preview" /> : (
          <>
      ...
      {items.map(item => (
        <div key={item.id} className="settings-toggle-row">
          ...
          <div className="settings-toggle-right">
            {item.hasInput && (
              <>
                <input className="settings-percent-input" type="number" value={item.value} onChange={e => setValue(item.id, e.target.value)} />
